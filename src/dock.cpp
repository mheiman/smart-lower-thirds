...
QScrollArea#LowerThirdContent QPushButton:hover { background: rgba(255,255,255,0.06); border-radius: 3px; }
	...
	// Browser Source size row + exclusive mode (inside settings panel)
	{
		auto *row = new QHBoxLayout();
		row->setSpacing(6);

		auto *lbl = new QLabel(tr("Browser Size:"), settingsPanel);

		browserWidthSpin = new QSpinBox(settingsPanel);
		browserWidthSpin->setRange(1, 16384);
		browserWidthSpin->setToolTip(tr("Width of the selected Browser Source"));
		browserWidthSpin->setFixedWidth(80);

		browserHeightSpin = new QSpinBox(settingsPanel);
		browserHeightSpin->setRange(1, 16384);
		browserHeightSpin->setToolTip(tr("Height of the selected Browser Source"));
		browserHeightSpin->setFixedWidth(80);

		exclusiveModeCheck = new QCheckBox(tr("Exclusive"), settingsPanel);
		...
		row->addWidget(new QLabel(tr("x"), settingsPanel));
...
void LowerThirdDock::rebuildList()
{
	...
	for (const auto &cfg : items) {
		...
		if (!cfg.profile_picture.empty() && !outDir.isEmpty()) {
			...
			QPixmap px(imgPath);
			if (!px.isNull()) {
				thumb->setPixmap(px.scaled(18, 18, Qt::KeepAspectRatioByExpanding, Qt::SmoothTransformation));
				hasThumb = true;
			}
		}
